...
- X / Twitter のポストへマウスを置くと自動判定
  ...
  - 本文のない投稿も、Xが提供する画像alt説明・動画説明があれば判定
- Xの表示モード
  - チワワだけ表示
  - 毒チワワ表示（毒エロチワワを含む）
  - エロチワワ表示（毒エロチワワを含む）
  - 毒エロチワワだけ表示
  - ポストの上端がブラウザ表示の上3/5付近まで来てから約0.18秒待って判定
  - 残すポストにも分類スタンプを表示し、判定後にアバター左上へ縮小固定
  - 対象外ポストは前面レイヤーで分類スタンプを押したあと非表示
...
- 投稿前チェックウィンドウ
  ...
  - コピー機能
- Xアカウント分析
  - `@username` / `username` / XプロフィールURLから対象アカウントを指定
  - プロフィールへ移動し、直近100件を自動スクロールで収集・判定
  - 毒指数・エロ指数と、各スコアのヒストグラムをカード表示
  - カード画像をコピーする機能と、結果文面（GitHubリンク付き）をコピーする機能
- モデルの事前準備
- モデルキャッシュ削除
...
アカウント分析では、直近の分析対象投稿について毒・エロを別々に集約します。Jigoku-no-Monの集約方針を参考にしつつ、単発の高スコアに引っ張られにくいよう平均を重視して、次の式で計算します。

```text
指数 = 0.50 × 平均 + 0.25 × p90 + 0.25 × 高スコア率
```

高スコア率は投稿スコアが `0.70` 以上の投稿割合です。指数はモデル出力から作る実験的な観測値であり、アカウントの人格・危険性・違法性を判定するものではありません。
